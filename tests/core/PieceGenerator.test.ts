...
import { Bishop, King, Knight, Mirror, Pawn, Queen, Rook } from "@lc/pieces";
...
describe("PieceGenerator", () => {
  ...
  test("createStandardPiece", () => {
    ...
    for (const zip of constructors.map((e, i) => { return [e, enumValues[i]] as [typeof King, PieceType] })) {
    ...
    for (const single of enumValues) {
      expect(() => Generator.createStandardPiece(data, single)).toThrowError(Error);
    }
  })

  test("createMirror", () => {
    const data: Generator.DirectedPieceData = {
      position: new BoardVector2d(1, 1),
      playerId: 0,
      board: board,
  ...
  test("createPawn", () => {
    const data: Generator.PawnPieceData = {
      position: new BoardVector2d(1, 1),
      playerId: 0,
      board: board,
